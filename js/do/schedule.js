...
//        console.log("ID Learner - " + idlearner);
        ...
        if($("#timetable_"+date).length) {
            ...
            var insHTML = "";
            var lessons = Schedule.loadLessons(date, idlearner);
            console.log(lessons);
            insHTML = Schedule.prepareLessonsUI(lessons, idschool, date);
                ...
                for(var idsubgroup in lessons[number]){
                    ...
                    if(lesson.idsubgroup!=0){
                        insHTML += '<td class="lesName" style="border-bottom:3px solid '+lesson.color+';">\
                                        <a href="study.php?school='+idschool+'&s=2&subject='+lesson.idsubject+'&group='+lesson.idsubgroup+'">\
