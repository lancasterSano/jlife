var SubjectsResponsible = {
	/*** ### SubjectSubgroup ### ***/
		expandSubject: function(post) {
			var selected = SubjectsResponsible.select_subjectExpanded();
			if(selected != undefined)
			{
				var IdDOM = selected.split("_"),
				c_post = IdDOM[2] + "_" + IdDOM[3] + "_" + IdDOM[4];
			}
			if(selected != undefined && c_post == post) SubjectsResponsible.turnSubject();
			else
			{
				SubjectsResponsible.turnSubject();
				...
				if(sps[2] != 0 )
				{
					...
					function(data){
						...
						{
							SubjectsResponsible.insertSubjectMaterial(post, Object.keys(data[1]).length > 0 ? data[1] : null);
						}
					},
				...
				else { SubjectsResponsible.insertSubjectMaterial(post, null); }
			}
		},
				...
				if(data != null)
				{
					for(var i in data)
					{
						...
						ins += 	"<li id='section_" + post + "_" + section.idSection + "'>\
									<span onclick='SubjectsResponsible.expandSection(\"" + post + "_" + section.idSection + "\");'>Раздел " + section.number + ": " + section.name + "</span>\
	...
	/*** ### Section ### ***/
		expandSection: function(post) {
			var selected = SubjectsResponsible.opened_sectionExpanded();
			if(selected != undefined)
			{
				var IdDOM = selected.split("_"),
				c_post = IdDOM[1] + "_" + IdDOM[2] + "_" + IdDOM[3] + "_" + IdDOM[4];
			}
			if(selected != undefined && c_post == post) SubjectsResponsible.turnSection();
			else
			{
				SubjectsResponsible.turnSection();
				...
				function(data){
					...
					{
						SubjectsResponsible.insertSectionParagraphs(post, Object.keys(data[1]).length > 0 ? data[1] : null);
					}
				},
				function(XMLHttpRequest, textStatus, errorThrown){ 
					// console.log("error from ajax: " + XMLHttpRequest.responseText);
		...
		insertSectionParagraphs: function(post, data) {
	        ...
	        var prepareHTML = "", ins = "", note = "",
	        	...
	        	schoolId = ($('li[id^=learner_]').attr("id").split("_"))[2];
