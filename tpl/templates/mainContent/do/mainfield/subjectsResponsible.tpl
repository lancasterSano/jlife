...
<div class="mainfield">     
	...
	{* header of table *}
        <div class="next_prev">
            <div class="prev">
                <a href="{$PROJECT_PATH}/pages/do/cabinet.php?learner={$numTab}&class={$PNCLASS["idPREVClass"]}&type=l&s=2">{$PNCLASS["namePREVClass"]}</a>
            </div>
            <div style="float:left;margin-left: 300px;">{$CURCLASS->name} класс</div>
            <div class="next">
                <a href="{$PROJECT_PATH}/pages/do/cabinet.php?learner={$numTab}&class={$PNCLASS["idNEXTClass"]}&type=l&s=2">{$PNCLASS["nameNEXTClass"]}</a>
        	</div>
        </div>
	{* body of table *}
        {if $isSubjects}
        <table class="subjects" cellspacing="0" >
            ...
            {foreach key=idGroup item=subject from=$subjects name=comments}
				...
				<tr id="study_{$stUnic}" onclick="SubjectsResponsible.expandSubject('{$stUnic}');">
					<td>
						<div>
							...
							{if $subject.material.idMaterial != NULL &&  $subject.material.idMaterial == $material_id}
								{foreach key=key item=section from=$material_sections name=sectionsS}
									<li id="section_{$stUnic}_{$key}">
										...
										<span onclick='SubjectsResponsible.expandSection("{$stUnic}_{$key}");'>Раздел {$section.number}:{$section.name}</span>
