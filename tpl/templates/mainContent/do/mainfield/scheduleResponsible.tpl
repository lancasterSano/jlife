    ...
    <div class="content">
        {if $isStudyDuration}
            <div class="next_prev">
                {if not $isfirstweek}
                <div class="prev">
                    <a href="cabinet.php?learner={$currentLearner->idLearner}&type=l&s=1&w={$nextprevweekyear.prevweek}">Предыдущая неделя</a>
                </div>
                {/if}
                {if not $isfirstweek}
                ...
                {if not $islastweek}
                <div class="next">
                    <a href="cabinet.php?learner={$currentLearner->idLearner}&type=l&s=1&w={$nextprevweekyear.nextweek}">Следующая неделя</a>
                </div>
                {/if}
            </div>
            ...
            <div class="day" id="day_{$day.dateForJS}">
                ...
                {/if}
                    ...
                    <div 
                        ...
                        onclick="ScheduleResponsible.loadLessonsUI('{$day.dateForJS}', '{$currentLearner->idLearner}', '{$currentLearner->idSchool}', {if $day.iscurrentstudyday}{$day.iscurrentstudyday}{else}0{/if}, {if $day.isnextstudyday}{$day.isnextstudyday}{else}0{/if});">
                ...
                <table id="timetable_{$day.dateForJS}" 
                    ...
                    <tbody>
                        ...
                        <tr>
                            ...
                            {else}
                            <td class="lesName" style="border-bottom:3px solid {*Цвет рамки*}{$lesson.color};">
                              <a href="cabinet.php?learner={$currentLearner->idLearner}&type=l&s=2&subject={$lesson.idsubject}&group={$lesson.idsubgroup}"><span>{*Название предмета*}{$lesson.subjectname}</span></a>
                            </td>
                            {/if}
                            ...
                            <td class="{if $lesson.islast}dzL{else}dz{/if}"> {*dz если предмет идет не последним в таблице, иначе dzL*}
                                <div> 
                                    {foreach $lesson.paragraphs as $paragraph}
                                        ...
                                        <a class="paragraphhref" href="paragraph.php?school={$currentLearner->idSchool}&paragraph={$paragraph.id}">
                                        ...
                                        {foreach $paragraph.partparagraphs as $partparagraph}
                                            <a class="partparagraphhref" href="paragraph.php?school={$currentLearner->idSchool}&paragraph={$paragraph.id}#s_{$partparagraph.id}">Часть {$partparagraph.number} </a>
                                        {/foreach}
                                         </a>
                                         </span>
                                    {/foreach}
                        ...
                        <tr>
                          <td style="text-align: center; font-weight: bold;">В этот день у Вашего ребёнка выходной!
                          </td>
                        </tr>
                        <tr>
