...
<div class="union">
    <div class="left_side">
        <div class="name">
            ...
            {if $ProfileLoad->ID != $ProfileAuth->ID}
                <div class="pageBut">
                    <div id="subscriber" onclick="Subscriber.addSubscriber({$ProfileLoad->ID})" class="{if $subscribe==0}notsubscribe{else if $subscribe>0}subscribe{/if}" ></div>
    ...
    <div class="right_side">
        <img id="ava" src="{$PROJECT_PATH}{$ProfileLoad->ProfilePathAvatar(1)}?ord={time()}" />
    </div>
</div><!-- end union-->