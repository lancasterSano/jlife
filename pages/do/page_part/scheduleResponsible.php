...
$r = $DB_DO->getRow(QSDO::$getCountStudyDays, $LearnerCurrent->idSchool);
$COUNT_WORK_DAYS = $r["countstudyday"];

// SET START EDUCATION AND END EDUCATION OF SEMESTRE
$r = $DB_DO->getRow(QSDO::$getActualStudyDuration, $LearnerCurrent->idSchool);
if(!$r){
    ...
    $notices['MW_SCHEDULE_LEARNER_HAS_NO_STUDYDURATION']["messages"][] = array(
        ...
        "text" => preg_replace("/%NAME/", $LearnerCurrent->FirstName, MW_SCHEDULE_RESPONSIBLE_HAS_NO_STUDYDURATION_TEXT)
    );
} else {
    ...
    $learnerSubgroups = $LearnerCurrent->getSubgroup();
    // MAIN LOOP
...
$block_include_head = $smarty->fetch("./include_head/do/USMF/inc_scheduleResponsible.tpl"); $smarty->assign('block_include_head', $block_include_head);
...
$block_mainField_tpl = $smarty->fetch("./mainContent/do/mainfield/scheduleResponsible.tpl");
$smarty->assign('block_mainField', $block_mainField_tpl);
