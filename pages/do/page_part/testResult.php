...
$r = $DB_DO->getAll(QSDO::$getMarksByDaysNSubjects, $Learner->idLearner, $monthToShow, $yearToShow);
if(!$r) $isTests = false;
else $isTests = true;
$subjectIDs = array();
$datesTry = array();
$resDatesTry = array();
$marks = array();
$subgroupsLearner = $Learner->getSubgroup();
if($subgroupsLearner){
    $isSubjects = true;
    ...
    $ratingQuery = $DB_DO->getAll(QSDO::$getRatingByOwnLearnerAndSubgroups, $subgroupsLearner, $Learner->idLearner);
    ...
    if($ratingQuery){
        ...
        foreach($ratingFull as $key => $value) {
            ...
            $position = $DB_DO->getOne(QSDO::$getPositionLearnerFromEachSubgroup, $value['subgroupS_id'], $Learner->idLearner);
    ...
    foreach ($subjectIDs as $subjectID) {
        ...
        $group = $DB_DO->getOne(QSDO::$getSubgroupBySubject, $subjectID, $Learner->idClass, $Learner->idLearner);
        $subjects[] = array(
            "id" => $subjectID,
            "name" => $r["name"],
            "learnergroup" => $group
        );
        foreach($resDatesTry as $dateTry => $value) {
    ...
    $notices['MW_TESTRESULT_LEARNER_HAS_NO_SUBJECTS']["messages"][] = array(
        ...
        "text" => MW_TESTRESULT_LEARNER_HAS_NO_SUBJECTS_TEXT
    );
}
...
$block_include_head = $smarty->fetch("./include_head/do/USMF/inc_testResult.tpl"); $smarty->assign('block_include_head', $block_include_head);
...
$tResMainField_tpl = $smarty->fetch("./mainContent/do/mainfield/testResult.tpl"); $smarty->assign('block_mainField', $tResMainField_tpl);
